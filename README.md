...
- **Attachments.** Attach images, PDFs, and text files from the composer — by button, drag-and-drop, or pasting a screenshot straight into the box. Images and PDFs go up as OpenRouter multimodal content parts; text files are inlined into the prompt, which works on every model. Large images are downscaled to 1568px on the longest edge and long text files are clipped at 100k characters, both flagged in the chat. Limits: 8 MB per file, 5 files per message. PDFs are extracted with OpenRouter's free `pdf-text` engine; set `pdf_ocr_engine` to `mistral-ocr` in extension storage to switch to paid OCR for scanned documents. Attachment payloads are dropped from history once answered, so a PDF is parsed and paid for once.
- **Voice input.** Dictate into the composer with Chrome's built-in speech recognition — free, no tokens, text appears as you speak. Chrome will not show a microphone prompt inside a side panel, so the first press opens a one-time grant tab; allow the mic there and the panel keeps working from then on.
