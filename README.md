...
- **Attachments.** Attach images, PDFs, and text files from the composer — by button, drag-and-drop, or pasting a screenshot straight into the box. Images and PDFs go up as OpenRouter multimodal content parts; text files are inlined into the prompt, which works on every model. Limits: 8 MB per file, 5 files per message. PDFs are extracted with OpenRouter's free `pdf-text` engine; set `pdf_ocr_engine` to `mistral-ocr` in extension storage to switch to paid OCR for scanned documents.
